...
class AuthProvider with ChangeNotifier{
 ...
 Future registerUser({
    ...
    try {
      await authservice.registerApi(body: body, context: context, setState: setState).then((value){
        if(value?.status?.httpCode=='200'){
         Fluttertoast.showToast(backgroundColor: Colors.green,msg: '${value?.status?.message}',textColor: Colors.white); 
  ...
  Future loginByMobile({
    ...
    try {
      await authservice.login(body: body, context: context, setState: setState).then((value){
        if(value?.status?.httpCode=='200'){
          Fluttertoast.showToast(backgroundColor: Colors.green,msg: '${value?.status?.message}',textColor: Colors.white);
          Navigator.push(context, MaterialPageRoute(builder: (context)=>OtpVerifyScreen(subtitle: value?.status?.message??'',resp: value,)));
       setState((){
        isLoding=false;
       });
       notifyListeners();
  ...
  Future<void> otpVerify(
    ...
    try {
      ...
      await authservice.otpVerifyByMobile(
          body: body, context: context, setState: setState).then((value){
            if(value?.status?.httpCode=='200'){
              SharepreferencClass().clearData();
              SharepreferencClass().saveUser(value?.data?.token, value?.data?.userId);
              Fluttertoast.showToast(msg: value?.status?.message??'',backgroundColor: Colors.green,textColor: Colors.white);
 ...
 Future loginByEmail({
   ...
   print({'email....': emailController.text});
    ...
    try {
      await authservice.loginByEmail(body: body, context: context, setState: setState).then((value){
        if(value?.status?.httpCode=='200'){
          Fluttertoast.showToast(backgroundColor: Colors.green,msg: '${value?.status?.message}',textColor: Colors.white);
          Navigator.push(context, MaterialPageRoute(builder: (context)=>OtpVerifyScreen(subtitle: value?.status?.message??'',resp: value,)));
  ...
  Future<void> otpVerifyByEmail(
    ...
    try {
      ...
      await authservice.otpVerifyEmail(
          body: body, context: context, setState: setState).then((value){
            if(value?.status?.httpCode=='200'){
              SharepreferencClass().clearData();
              SharepreferencClass().saveUser(value?.data?.token, value?.data?.userId);
   ...
   var getid = pref.getInt('userId');
    ...
    try {
      ...
      await authservice.getUserByIdApi(query: query, context: context, setState: setState).then((value){
        if(value?.status?.httpCode=='200'){
          setState((){
            userModel=value;
            isLoding=false;
  ...
  Future<void> editProfileById(
   ...
   var getid = pref.getInt('userId');
    ...
    try {
      ...
      await authservice.editProfileByIdApi(body: body, userId: getid, context: context, setState: setState).then((value){
        if(value?.status?.httpCode=='200'){
           Fluttertoast.showToast(backgroundColor: Colors.green,msg: '${value?.status?.message}',textColor: Colors.white);
          Navigator.push(context, MaterialPageRoute(builder: (context)=> Otpverifybyprofilescreen(editResp: value,)));
        setState((){
          isLoding=false;
        });
        }
  ...
  Future<void> getCategoryList(
    ...
    try {
      ...
      await authservice.getCategoryListApi(query: query, context: context, setState: setState).then((value){
        if(value?.status?.httpCode=='200'){
